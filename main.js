...
function convertCase(selectedChoice) {
  if (selectedChoice === 'paper') return 'Paper';
  if (selectedChoice === 'scissors') return 'Scissors';
  return 'Rock';
}
...
function win(user, computer) {
  ...
  roundStatus.classList.add('win-green');
  setTimeout(() => roundStatus.classList.remove('win-green'), 500);
}
...
function loses(user, computer) {
  ...
  const roundStatus = document.getElementById(user);
  roundStatus.classList.add('lose-red');
  setTimeout(() => roundStatus.classList.remove('lose-red'), 500);
}
...
function draw(user, computer) {
  ...
  const roundStatus = document.getElementById(user);
  roundStatus.classList.add('draw-gray');
  setTimeout(() => roundStatus.classList.remove('draw-gray'), 500);
}

// The core game functions that set up and determine the games actual logic (rules)
...
// This function creates and adds an eventlistener to the rock, paper and scissors html element and then passes the value of that element to the game function
function main() {
